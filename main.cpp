...
public:
   ...
   LaneDetect(Mat startFrame)
   {
      ...
      minSize = 0.0003 * (currFrame.cols*currFrame.rows);  //min size of any region to be selected as lane
   ...
   //float get_Angle(float vp_x, float vp_y, float middle_point_x, float middle_point_y)
   //{
   //   //y==0으로 준다.
   //   float Angle = 0;
   //   float distance_a = vp_x - middle_point_x;
   //   float distance_b = middle_point_y - vp_y;
   //   Angle = atan2f(distance_b, distance_a) * 180 / 3.14 - 90;
   //   return Angle;
   //}
               ...
               if (bounding_length > longLane || bounding_width > longLane)
               {
                  ...
                  for (int q = 0; q < contours.size(); q++) //vector<vector<point>>
                  {
                     if (contours[q].size() >= 65) {
                        ...
                        //circle(temp2, (contours[q][0]), 10, (0, 0, 255), -1);
...
//	bluetoothmod.connect("COM8");
	...
	while (1) { //오픈에 성공한 경우 sendCommand()를 통해 계속적으로 데이터를 전송한다. 전송에 실패 할 경우 failed 메시지를 출력한다.
		...
		//for (int i = 0; i < 4; i++) {
			//cin >> test;
		//	if (!bluetoothmod.sendCommand(test))
		//	{
		...
		//}
		//
		//if (bluetoothmod.recvCommand(test)) {
		//	cout << "전달받은 값" << test << endl;
		//}
		//else {
